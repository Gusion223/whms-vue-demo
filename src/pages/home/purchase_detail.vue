...
<!--      // 顶部搜索栏-->
      <div class="top-search">
        ...
        <el-button type="success" style="margin-left: 5px" @click="openAddForm">添加入库商品信息细则</el-button>
      </div>
      ...
      <div>
        <el-dialog title="添加新进货记录" v-model="addFormVisible">
          <el-form :model="addForm" :rules="formRules" ref="addFormRef">
            <el-form-item prop="poid" :label-width="addFormLabelWidth" label="订单id">
              <el-input v-model="addForm.poid"></el-input>
            </el-form-item>
            ...
            <el-form-item prop="pdtotalCost" :label-width="addFormLabelWidth" label="总成本">
              <el-input v-model="addForm.pdtotalCost" controls-position="right" style="width: 100%"></el-input>
            </el-form-item>


            <el-button type="info" @click="addFormVisible=false">取消</el-button>
            <el-button type="primary" @click="tryAdd">添加新的入库细则</el-button>
          </el-form>
        </el-dialog>
      </div>
...
<!--      // 修改信息的表单-->
      <div>
        <el-dialog title="修改入库细则信息" v-model="updateFormVisible">
          <el-form :model="updateForm" :rules="formRules" ref="updateFormRef">
            <el-form-item prop="poid" :label-width="updateFormLabelWidth" label="订单id">
              <el-input v-model="updateForm.poid" ></el-input>
            </el-form-item>
            ...
            <el-form-item prop="pdtotalCost" :label-width="updateFormLabelWidth" label="总成本">
              <el-input v-model="updateForm.pdtotalCost" controls-position="right" style="width: 100%"></el-input>
            </el-form-item>
...
<script setup>
  ...
  const tryAdd = async ()=>{
    ...
    try{
      ...
      // api处理
      let res = await ApiAddPurchaseDetail(
          addForm.value.poid,
          addForm.value.gid,
  ...
  const tryUpdate = async ()=>{
    try{
      ...
      let res = await ApiUpdatePurchaseDetail(
      ...
      ElMessage({message:`入库记录细则${updateForm.value.poid}添加成功`, type:"success"})
      await loadData()
    }catch(e){
      // ElMessage({message:"用户表单未通过校验要求", type:'warning'})
      return
  ...
  const tryDelete = async (old_data) => {
    try {
      let res = await ApiDeletePurchaseDetail(old_data.poid,old_data.gid)
      if (res.data.status !== 200)
        ElMessage({message: res.data.msg, type: "warning"})
      else{
        ElMessage({message: `成功删除入库记录细则${old_data.poid}}`, type: "success"})
        await loadData()
      }
    } catch (e) {
