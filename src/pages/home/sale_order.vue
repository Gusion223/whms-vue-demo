...
<!--      // 顶部搜索栏-->
      <div class="top-search">
        ...
        <el-button type="success" style="margin-left: 5px" @click="openAddForm">销售信息登记</el-button>
      </div>

<!--      // 添加信息的表单-->
      <div>
        <el-dialog title="添加新销售记录" v-model="addFormVisible">
          <el-form :model="addForm" :rules="formRules" ref="addFormRef">
            ...
            <el-button type="primary" @click="tryAdd">添加新的销售记录</el-button>
          </el-form>
        </el-dialog>
      </div>
...
<!--      // 修改信息的表单-->
      <div>
        <el-dialog title="修改销售记录信息" v-model="updateFormVisible">
          <el-form :model="updateForm" :rules="formRules" ref="updateFormRef">
            ...
            <el-button type="primary" @click="tryUpdate">修改当前销售信息</el-button>
          </el-form>
        </el-dialog>
      </div>
...
<script setup>
  ...
  const tryAdd = async ()=>{
    ...
    try{
      ...
      ElMessage({message:`销售记录${addForm.value.soid}添加成功`, type:"success"})
      await loadData()
    }catch(e){
      console.log(e)
    }
  ...
  const tryUpdate = async ()=>{
    try{
      ...
      ElMessage({message:`销售记录总则${updateForm.value.soid}添加成功`, type:"success"})
      await loadData()
    }catch(e){
      // ElMessage({message:"用户表单未通过校验要求", type:'warning'})
      return
  ...
  const tryDelete = async (old_data) => {
    try {
      ...
      if (res.data.status !== 200)
        ...
        ElMessage({message: `成功删除销售记录${old_data.soid}}`, type: "success"})
        await loadData()
      }
    } catch (e) {
